...
public class PronounsSetup extends Pronouns {
    ...
    public List<String> getPronouns() {
        if (pc.getLanguage() == Language.getLangFile("en"))
            return PRONOUNS_EN.get(getPronounNum());
        else if (pc.getLanguage() == Language.getLangFile("de"))
            return PRONOUNS_DE.get(getPronounNum());
